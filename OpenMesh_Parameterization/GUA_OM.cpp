...
double area(OMT::Point A,OMT::Point B,OMT::Point C)
{
	double l1,l2,l3;
	double s;
	l1 = (A-B).length();
...
void Tri_Mesh::Select_Face( int x, int y)
{
	...
	for( OMT::FIter f_it = faces_begin() ; f_it != faces_end() ; ++f_it )
	{
		...
		{
			select_fh = f_it.handle();
			error = sum-temp_area;
		}
		/*
		for ( OMT::FVIter fv_it = fv_iter( f_it ) ; fv_it ; ++fv_it )
		{
			  temp_len += (point(fv_it)-pt).length();
		}
...
void Tri_Mesh::Select_Point( int x, int y)
{
	GLdouble M[16], P[16]; GLint V[4];
	GLfloat      depth_buffer;
	glGetDoublev(GL_MODELVIEW_MATRIX, M);
